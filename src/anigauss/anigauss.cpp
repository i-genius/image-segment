...
Any commercial use or _redistribution of this software requires a license from
...
legally requi_red, to do the following:
...
including _redistributable fixes or workarounds.
...
static void f_iir_xline_filter(SRCTYPE *src, DSTTYPE *dest, int sx, int sy, DSTTYPE *filter);
static void f_iir_yline_filter(SRCTYPE *src, DSTTYPE *dest, int sx, int sy, DSTTYPE *filter);
static void f_iir_tline_filter(SRCTYPE *src, DSTTYPE *dest, int sx, int sy, DSTTYPE *filter, DSTTYPE tanp);
static void f_iir_derivative_filter(SRCTYPE *src, DSTTYPE *dest, int sx, int sy, DSTTYPE phi, int order);
...
void YvVfilterCoef(DSTTYPE sigma, DSTTYPE *filter)
{
	...
	/* initial values */
	DSTTYPE m0 = 1.16680, m1 = 1.10783, m2 = 1.40586;
	DSTTYPE m1sq = m1*m1, m2sq = m2*m2;
...
static void f_iir_xline_filter(SRCTYPE *src, DSTTYPE *dest, int sx, int sy, DSTTYPE *filter)
{
	int      i, j;
	DSTTYPE   b1, b2, b3;
	DSTTYPE   pix, p1, p2, p3;
	DSTTYPE   sum, sumsq;
	DSTTYPE   iplus, uplus, vplus;
	DSTTYPE   unp, unp1, unp2;
	DSTTYPE   M[9];
...
static void
f_iir_yline_filter(SRCTYPE *src, DSTTYPE *dest, int sx, int sy, DSTTYPE *filter)
{
	DSTTYPE   *p0, *p1, *p2, *p3, *pswap;
	DSTTYPE   *buf0, *buf1, *buf2, *buf3;
	DSTTYPE   *uplusbuf;
	int      i, j;
	DSTTYPE   b1, b2, b3;
	DSTTYPE   pix;
	...
	uplusbuf = (DSTTYPE*)malloc(sx*sizeof(*uplusbuf));

	buf0 = (DSTTYPE*)malloc(sx*sizeof(*buf0));
	buf1 = (DSTTYPE*)malloc(sx*sizeof(*buf1));
	buf2 = (DSTTYPE*)malloc(sx*sizeof(*buf2));
	buf3 = (DSTTYPE*)malloc(sx*sizeof(*buf3));
...
static void f_iir_tline_filter(SRCTYPE *src, DSTTYPE *dest, int sx, int sy,
	DSTTYPE *filter, DSTTYPE tanp)
{
	DSTTYPE   *p0, *p1, *p2, *p3;
	DSTTYPE   *buf0, *buf1, *buf2, *buf3;
	DSTTYPE   *uplusbuf;
	...
	buf0 = (DSTTYPE*)malloc((sx + sy*tanp + 2)*sizeof(*buf0));
	buf1 = (DSTTYPE*)malloc((sx + sy*tanp + 2)*sizeof(*buf1));
	buf2 = (DSTTYPE*)malloc((sx + sy*tanp + 2)*sizeof(*buf2));
	buf3 = (DSTTYPE*)malloc((sx + sy*tanp + 2)*sizeof(*buf3));
